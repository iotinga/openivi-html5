...
void Phone::InitOfono()
{
    ...
    if (ofonoInterface != NULL)
    {
        ...
        connect(ofonoInterface, SIGNAL(ModemAdded(const QDBusObjectPath &, const QVariantMap &)), this,
                SLOT(OnModemAdded(const QDBusObjectPath &, const QVariantMap &)));
        connect(ofonoInterface, SIGNAL(ModemRemoved(const QDBusObjectPath &)), this,
        ...
        for (int i = 0; i < modems.size(); i++)
        {
            ...
            if (modemIface != NULL)
            {
                connect(modemIface, SIGNAL(PropertyChanged(QString, QDBusVariant)), this,
...
void Phone::InitModem(OfonoModem &modemInfo)
{
    ofonoVoiceCallManager =
        new OrgOfonoVoiceCallManagerInterface("org.ofono", modemInfo.objectPath.path(), QDBusConnection::systemBus());
    if (ofonoVoiceCallManager)
    {
        connect(ofonoVoiceCallManager, SIGNAL(CallAdded(QDBusObjectPath, QVariantMap)), this,
                SLOT(OnCallStarted(QDBusObjectPath, QVariantMap)));
        connect(ofonoVoiceCallManager, SIGNAL(CallRemoved(QDBusObjectPath)), this, SLOT(OnCallClosed(QDBusObjectPath)));
        // Init Network Info
        ofonoNetworkInfo = new OrgOfonoNetworkRegistrationInterface("org.ofono", modemInfo.objectPath.path(),
        ...
        if (ofonoNetworkInfo != NULL)
        {
            ...
            connect(ofonoNetworkInfo, SIGNAL(PropertyChanged(QString, QDBusVariant)), this,
                    SLOT(OnNetworkInfoChanged(QString, QDBusVariant)));
        }
        // Init handsfree interface to read battery level
        ofonoHandsfreeInfo =
            new OrgOfonoHandsfreeInterface("org.ofono", modemInfo.objectPath.path(), QDBusConnection::systemBus());
        if (ofonoHandsfreeInfo != NULL)
        {
            ...
            connect(ofonoHandsfreeInfo, SIGNAL(PropertyChanged(QString, QDBusVariant)), this,
                    SLOT(OnHandsfreePropertyChanged(QString, QDBusVariant)));
        }
        // Init volume controls
        ofonoVolume =
            new OrgOfonoCallVolumeInterface("org.ofono", modemInfo.objectPath.path(), QDBusConnection::systemBus());
        if (ofonoVolume != NULL)
        {
            ...
            connect(ofonoVolume, SIGNAL(PropertyChanged(QString, QDBusVariant)), this,
                    SLOT(OnVolumePropertyChanged(QString, QDBusVariant)));
        }
...
void Phone::OnModemRemoved(const QDBusObjectPath &path)
{
    ...
    for (int mdm = 0; mdm < ofonoModemInterfaces.size(); mdm++)
    {
        ...
        if (modemIface)
        {
            ...
            {
                disconnect(modemIface, SIGNAL(PropertyChanged(QString, QDBusVariant)), this,
                           SLOT(OnModemPropertyChanged(QString, QDBusVariant)));
...
void Phone::OnCallStarted(const QDBusObjectPath &path, const QVariantMap &properties)
{
    ...
    if ((callNumberIter != properties.end()) && (callStatusIter != properties.end()))
    {
        ...
        ofonoVoiceCall =
            new OrgOfonoVoiceCallInterface("org.ofono", currentCallPath.path(), QDBusConnection::systemBus());
        if (ofonoVoiceCall != NULL)
        {
            connect(ofonoVoiceCall, SIGNAL(PropertyChanged(QString, QDBusVariant)), this,
